(function ($) {
    ...
    function getBlog() {
        ...
        let icons = ['bxl-dribbble','bx-file', 'bx-tachometer'];
        let colors = ['blue', 'orange', 'pink'];
        let hexColors = ['#47aeff', '#ffa76e', '#e80368']
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.body.found > 0) {

                    $.each(posts, function(i, e) {

                        let postDate = e.date;
                        let date = new Date(postDate);
                        let postExcerpt = limitText(e.excerpt, 100);

                        let data = {
                            'post_id'      : e.ID,
                            'post_author'  : e.author.name,
                            'post_content' : e.content,
                            'post_exerpt'  : postExcerpt.replace('<p>', '').replace('</p>', ''),
                            'post_date'    : date.toISOString().split('T')[0].replace(/-/g, '-')
                        };
